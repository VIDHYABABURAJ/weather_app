...
class _WeatherConditionState extends State<WeatherCondition> {
  ...
  Future<void> _getCurrentPosition() async {
    try {
      ...
      _climate = await weathercontroller.getData(
          latitude: position.latitude.toString(),
          longitude: position.longitude.toString());
      print("xyz");

      _forecast = await weathercontroller.getHourlydata(
          latitude: position.latitude.toString(),
          longitude: position.longitude.toString());
      print("abc");
      ...
      _dailyData = await weathercontroller.getDaily(
          cityname: placemarks[0].name.toString());
  ...
  List<Widget> getHourlyWidgetlist() {
    List<Widget> widgetlist = _forecast!
        .map(
          (e) => SizedBox(
            width: 150,
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Padding(
                                padding:
                                    const EdgeInsets.only(top: 20, left: 200),
                                child: Text(
                            ...
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => LocationScreen(
                                    weather: _climate!.weather![0].description
                                        .toString(),
                                    min: kelvintoTemp(
                                            temp: _climate!.main!.tempMin)
                                        .toStringAsFixed(0),
                                    max: kelvintoTemp(
                                            temp: _climate!.main!.tempMax)
                                        .toStringAsFixed(0),
                                    sunset: _climate!.sys!.sunset,
                                    sunrise: _climate!.sys!.sunrise,
                                    temp: _dailyData!.current!.tempC,
                                    uv: _dailyData!.forecast!.forecastday,
                                  ),
                                ));
                          },
                          icon: Icon(
